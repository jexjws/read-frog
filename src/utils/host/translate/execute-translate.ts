...
import { alibabaBailianMTTranslate } from "./api/alibaba-bailian-mt"
...
export async function executeTranslate(
  ...
  if (isNonAPIProvider(provider)) {
    ...
    else if (provider === "alibaba-bailian-mt") {
      translatedText = await alibabaBailianMTTranslate(text, sourceLang, targetLang, providerConfig, options)
    }
  }
